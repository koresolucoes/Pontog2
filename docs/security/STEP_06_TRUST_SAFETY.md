...
2. manter as migrations 06A–06H aplicadas;
3. não restaurar grants anônimos, coordenadas exatas ou credenciais padrão;
...
- Social SECURITY DEFINER: `anon_exec=false`.
- `admins/admin_audit_logs`: sem grants para `anon/authenticated`.
- bloqueio impede nova mensagem.
